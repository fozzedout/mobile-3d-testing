...
The four courses share a `FlightRig` (the exact touch-nav controls tuned in Space Sim — same GUI, same toggles) and a `CourseTimer` (countdown → racing → finished, hit penalties, localStorage best time).
